  ...
  <main class="d-flex justify-content-center align-items-center h-75">
    <form action="{{ route('boards.store') }}" method="post" style="width: 300px" enctype="multipart/form-data"> @csrf
      ...
      <div class="mb-3">
        ...
        <input type="text" id="b_title" name="b_title" class="form-control" value="{{ old('b_title') }}">
      </div>
      <div class="mb-3">
        <label for="b_content" class="form-label">내용</label>
        <input type="text" id="b_content" name="b_content" class="form-control" value="{{ old('b_content') }}">
      </div>
      <div class="mb-3">
        <label for="b_img" class="form-label">이미지</label>
        <input type="file" id="b_img" name="file" class="form-control">
      </div>
